...
export default async function ParentPage({ searchParams }: { searchParams: Promise<{ child?: string }> }) {
  ...
  const selectedEnrollmentRows = selectedChildId ? data.enrollments.filter((row) => row.student_id === selectedChildId) : [];
  const selectedCohortIds = new Set(selectedEnrollmentRows.map((row) => row.cohort_id));
  ...
  return (
    <main dir="rtl" className="min-h-dvh bg-[#f6f5fb] text-[#17152b]">
      ...
      <div className="mx-auto max-w-6xl px-4 py-8 md:px-8">
        ...
        {selectedChild ? <section className="mt-6 flex flex-col gap-4 rounded-2xl border border-[#e8e5ef] bg-white p-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs font-bold text-[#6547d9]">الملف الحالي</p><h2 className="mt-1 text-lg font-extrabold">{selectedChild.full_name}</h2><p className="mt-1 text-xs text-[#6f6a80]">{selectedChild.grade ?? "المرحلة غير محددة"} · {selectedChild.code}{relationship ? ` · ${relationship}` : ""}</p></div><div className="rounded-xl bg-[#f6f4ff] px-4 py-3 text-sm"><span className="text-xs text-[#6f6a80]">مستحقات هذا الابن</span><b className="mt-1 block text-lg">{selectedDueTotal.toLocaleString("ar-EG")} ج</b></div></section> : null}
        ...
        <section className="mt-6 grid gap-5 lg:grid-cols-2">
          <article className="card p-5 md:p-6"><h2 className="font-extrabold">أبنائي</h2><div className="mt-4 space-y-3">{data.children.map((child)=><Link href={`/parent?child=${encodeURIComponent(child.id)}`} key={child.id} className={`block rounded-xl border p-4 transition ${child.id===selectedChildId?"border-[#6547d9] bg-[#f7f5ff]":"border-[#eeeaf6] hover:border-[#cfc7ee]"}`}><div className="flex items-center justify-between gap-3"><div><b className="text-sm">{child.full_name}</b><span className="mt-1 block text-xs text-[#6f6a80]">{child.grade ?? "المرحلة غير محددة"} · {child.code}</span></div><span className={`rounded-full px-3 py-1 text-xs font-bold ${child.id===selectedChildId?"bg-[#6547d9] text-white":"bg-emerald-50 text-emerald-700"}`}>{child.id===selectedChildId?"الحالي":"عرض"}</span></div></Link>)}</div></article>
