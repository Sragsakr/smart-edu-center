...
import { NewInviteAccountForm, SwitchInviteAccountForm } from "./invite-client";
          ...
          <div className="mt-6">
            ...
            {invitation.accountExists ? (
              <div className="mt-4 rounded-2xl border border-red-200 bg-red-50 p-4 text-sm leading-7 text-red-700">
                هذا البريد مسجل بالفعل في النظام، لذلك هذه الدعوة القديمة غير قابلة للاستخدام. اطلب من إدارة مساحة العمل إلغاءها واستخدام بريد غير مسجل.
              </div>
            ) : user ? (
              <>
                <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-7 text-amber-800">أنت مسجل الدخول حاليًا بحساب آخر. سجّل الخروج أولًا لإنشاء الحساب الجديد المرتبط بهذه الدعوة.</div>
                <SwitchInviteAccountForm token={token} />
              </>
            ) : (
              <>
                <p className="mt-4 text-sm leading-7 text-[#6f6a80]">أنشئ كلمة مرور للحساب الجديد. بعد ذلك سيتم إنشاء الحساب وقبول الدعوة وربط الصلاحية والدخول إلى مساحة العمل تلقائيًا.</p>
