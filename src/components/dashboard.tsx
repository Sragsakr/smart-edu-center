...
function Placeholder({active}:{active:string}) { const item=nav.find(x=>x[0]===active); const Icon=item?.[1]||BookOpen; return <div className="enter card grid min-h-[520px] place-items-center p-8 text-center"><div><span className="mx-auto grid size-20 place-items-center rounded-3xl bg-[#eeeaff] text-[#6547d9]"><Icon size={34}/></span><h2 className="mt-5 text-xl font-extrabold">{active}</h2><p className="mx-auto mt-2 max-w-md text-sm leading-7 text-[#7f7b8f]">هذه الوحدة ضمن هيكل الـMVP وجاهزة للربط ببيانات PostgreSQL في المرحلة التالية.</p><button className="mt-6 rounded-xl bg-[#6547d9] px-5 py-3 text-xs font-bold text-white">ابدأ الإعداد</button></div></div>; }
