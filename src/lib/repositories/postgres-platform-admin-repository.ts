...
export class PostgresPlatformAdminRepository implements PlatformAdminRepository {
  /**
   * ينفّذ استعلامًا إداريًا داخل نطاق المنصة بعد التحقق من `platform_admins`.
   *
   * كل قراءة عبر المساحات تمر من هنا، فلا يوجد استعلام إداري واحد يعمل خارج نطاق
   * مصرّح به. المرجع: docs/adr/0007.
   */
  private async inPlatformScope<Result>(operation: (sql: SqlExecutor) => Promise<Result>): Promise<Result> {
    const outcome = await withPlatformScope(({ sql }) => operation(sql));
    if (outcome.status === "unauthenticated") throw new Error("platform admin authentication required");
    if (outcome.status === "forbidden") throw new Error("platform admin privilege required");
  ...
  async getCurrentPlatformAdminAccess(): Promise<RepositoryPlatformAdminAccess> {
    const outcome = await withPlatformScope(async ({ user }) => user);
    if (outcome.status !== "authorized") return outcome;
    return { status: "authorized", user: outcome.value };
  }

  async getOverviewMetrics(): Promise<RepositoryOverviewMetrics> {
    return this.inPlatformScope(async (sql) => {
  ...
  async getPlatformReport(): Promise<RepositoryPlatformReport> {
    return this.inPlatformScope(async (sql) => {
      const [tenantTypes, requestStatuses, membershipRoles, studentStatus] = await Promise.all([
        sql.query<{ label: string; value: number }>(`
        ...
        sql.query<{ label: string; value: number }>(`
          ...
          group by role
        `),
        sql.query<{ active_students: number; inactive_students: number }>(`
          select
