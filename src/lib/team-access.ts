...
import { capabilitiesForRole, type CapabilityMap, type MemberRole } from "@/lib/authorization/policy";
...
export async function getTeamWorkspaceData(requestedTenantId?: string): Promise<TeamWorkspaceData | null> {
  ...
  const { data: memberships, error: membershipsError } = await supabase.from("memberships").select("tenant_id,role,active").eq("user_id", user.id).eq("active", true);
  ...
  const workspaces = memberships.map((membership) => ({ tenant_id: membership.tenant_id, role: membership.role as MemberRole, tenant_name: nameByTenant.get(membership.tenant_id) ?? "مساحة عمل" }));
  ...
  const canReadInvitations = capabilities["invitations.read"] === "allow";

  const { data: memberRows, error: memberError } = await supabase.from("memberships").select("user_id,role,active,created_at").eq("tenant_id", chosen.tenant_id).order("created_at", { ascending: true });
  ...
  const members = await Promise.all((memberRows ?? []).map(async (row) => {
    ...
    return { user_id: row.user_id, role: row.role as MemberRole, active: row.active, created_at: row.created_at, email: data.user?.email ?? "بريد غير متاح" };
  }));

  let invitations: TeamWorkspaceData["invitations"] = [];
  if (canReadInvitations) {
    const now = new Date().toISOString();
    const { error: expiryError } = await supabase.from("invitations").update({ status: "expired", updated_at: now }).eq("tenant_id", chosen.tenant_id).eq("status", "pending").lt("expires_at", now);
    if (expiryError) throw new Error("تعذر تحديث حالة الدعوات المنتهية");

    const { data, error } = await supabase.from("invitations").select("id,invitee_email,role,status,expires_at,last_sent_at,created_at").eq("tenant_id", chosen.tenant_id).order("created_at", { ascending: false });
  ...
  return { currentUserId: user.id, tenant: { id: chosen.tenant_id, name: chosen.tenant_name }, role: chosen.role, capabilities, workspaces, members, invitations };
}
