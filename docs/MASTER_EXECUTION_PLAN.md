...
CURRENT_TASK: P02-04 — ربط UI بـserver-derived capabilities للعرض/التعطيل فقط
NEXT_TASK: P02-05 — Role × Resource × Action suite موجبة وسالبة
...
| 2026-09-11 | `P02-03` | عزل RLS مُلزم بـ132 سياسة و`FORCE RLS` على 33 جدولًا، وأدوار منفصلة (`saboraty_app` للـruntime، المالك للتطبيق والبذر) بعد إثبات أن الـsuperuser يتجاوز السياسات، وإعادة توصيل ~15 ملف DAL لفتح سياق معاملة بـ`set_config(..., true)`. 12 اختبار إلزام RLS بدور التطبيق + 105 integration + 123 unit، وتحقق بصري فعلي: الدخول وبوابتا الطالب وولي الأمر ولوحة المنصة تعمل تحت RLS مُلزم. | `P02-04` ربط UI بالقدرات |
