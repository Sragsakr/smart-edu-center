...
```text
CURRENT_PHASE: PHASE-02 — الصلاحيات والملكية والجلسات والعزل
CURRENT_TASK: P02-08 — إدارة الجلسات والأجهزة: list، revoke one، logout all
NEXT_TASK: P02-09 — تعطيل membership يمنع الإجراءات الحساسة فورًا دون التأثير على Tenants أخرى
...
LAST_VERIFIED_AUTOMATED_GATE: `npm run check` كامل ناجح (lint + typecheck + 177 unit + 182 integration على 14 ملفًا + migrations + secrets + build + client-secrets + retired-runtime-dependency) بعد `P01-10..P01-15` و`P02-00..P02-07-H` و`P02-11`، مع تحقق حي على قاعدة تطوير نظيفة لإنشاء الحساب والجلسة ومسار دعوة الفريق
```
...
- [x] `P02-07` **نقل الملكية وحماية آخر مالك، مع confirmation وaudit — منفَّذ ومختبر حيًّا.** trigger مؤجّل `memberships_require_active_owner` يرفض أي نهاية معاملة بلا مالك نشط، و`tenant-ownership.ts` ينقل في معاملة واحدة يرفع المنقول إليه قبل خفض القائم. 21 اختبارًا للخدمة والقيد. **تحقق حي:** النقل نجح (المالك ← مشرف، والمحاسب ← مالك)، والتأكيد الخاطئ رُفض والملكية لم تتغير، والقسم ظهر للمالك الجديد وحُجب عن المشرف السابق، وسجل التدقيق حمل المالكين السابق والجديد.
- [x] `P02-07-H` **إصلاح عاجل لمسار إنشاء الحساب ودعوات الفريق بعد الاختبار اليدوي.** `INSERT ... RETURNING` على `app_users` كان يفشل تحت RLS: الإضافة مسموحة بلا جلسة، لكن قراءة الصف العائد تتطلب هوية غير موجودة بعد. أُصلح بتوليد UUID في التطبيق والإدراج بلا `RETURNING`، لا بإضعاف سياسة SELECT. وكُشف أن معاينة دعوة الفريق كانت تقرأ جدولًا scoped بلا مساحة فترجع صفر صفوف؛ أُضيفت الدالة المحدودة `private.invitation_by_token` ثم الدخول إلى المساحة والقفل وإعادة التحقق. شمل الإصلاح التسجيل المباشر والتسجيل من دعوة والحساب القائم، واختبارين RLS جديدين بدور التطبيق. تحقق حي على قاعدة نظيفة: الحساب + credential + session، ومسار الدعوة حتى عضوية `teacher` وحالة `accepted`.
- [x] `P02-07-R` **علّة معمارية حاسمة كشفها مسبار مقصود.** `context.sql` بعد رجوع دوال التفويض يعمل **بلا سياق**، لأن `set_config(..., true)` هي `LOCAL` فتُصفَّر عند `COMMIT`/`ROLLBACK` — فيرجع **صفر صفوف مع دور التطبيق** بينما ينجح خطأً مع دور المالك الذي يتجاوز RLS. هذا هو سبب فشل أول محاولة نقل. الإصلاح على مستويين: (١) حارس في `postgres-sql-executor.ts` يرفض أي استعلام أو تعديل نطاق بعد نهاية المعاملة بخطأ `access context expired` بدل نتيجة فارغة صامتة؛ (٢) تحويل `withTenantContext` و`requireTenantCapability` و`requireTenantCapabilityWithScope` إلى **نمط callback** يجعل عمر السياق صريحًا ومستحيل الإساءة. وقد كشف الحارس نفسه أثناء الإصلاح ثلاث حالات استخدام خاطئة إضافية في القاعدة القائمة.
...
| 2026-09-12 | 2026-09-12 | 2026-09-12 | `P02-07-H` | إصلاح عاجل: إنشاء الحساب كان مكسورًا كليًا بدور التطبيق لأن `INSERT ... RETURNING` يحتاج سياسة SELECT قبل وجود هوية. توليد UUID + INSERT بلا RETURNING، وإصلاح معاينة/قبول دعوات الفريق بدالة token محدودة ثم tenant scope. 182 integration، وتحقق حي على قاعدة نظيفة. | `P02-08` إدارة الجلسات |
| 2026-09-12 | 2026-09-12 | 2026-09-12 | `P02-07` | نقل الملكية في معاملة واحدة + trigger مؤجّل يمنع مساحة بلا مالك نشط. **علّة معمارية:** `context.sql` بعد رجوع دوال التفويض يعمل بلا سياق (إعدادات `LOCAL`) فيرجع صفر صفوف مع دور التطبيق وينجح خطأً مع دور المالك. أُصلح بحارس `access context expired` + تحويل كل دوال التفويض إلى نمط callback. 21 اختبارًا وتحقق حي كامل. | `P02-08` إدارة الجلسات |
