...
تعمل `/platform-admin` كـControl Plane فعلية تعتمد على بيانات Supabase الحالية، وتشمل نظرة عامة، طلبات الحسابات، استعادة كلمات المرور، المساحات، المستخدمين والعضويات، تقارير SaaS، وسجل التدقيق. جميع الأرقام والقوائم والمخططات ناتجة من استعلامات Server-only وليست قيم Demo ثابتة في الواجهة. يمكن للمشرف تعليق Tenant أو إعادة تفعيله؛ تتغير عضوياته وتُسجل العملية في `platform_audit_logs`.
