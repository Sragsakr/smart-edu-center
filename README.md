...
### إنشاء الحساب ودعوات الفريق

إنشاء الحساب مسموح قبل وجود هوية، لكن قراءة صف `app_users` محمية بالهوية. لذلك يولّد التطبيق UUID، ثم ينفّذ `INSERT` **بلا `RETURNING`**؛ استخدام `RETURNING` هنا يحتاج سياسة SELECT ويفشل تحت RLS. لا تُوسَّع سياسة القراءة لحل ذلك، لأنها ستكشف صف هوية بلا مبرر.

ودعوة الفريق تُحل من الرمز عبر الدالة المحدودة `private.invitation_by_token`، لأن الزائر لا يعرف المساحة بعد. بعد حل الرمز يدخل التنفيذ سياق المساحة، يقفل صف الدعوة، ويعيد التحقق قبل إنشاء العضوية. هذا هو نفس نمط دعوات الطالب وولي الأمر، مع فصل المجالين.
